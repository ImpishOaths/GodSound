/* Sonic library
   ...
   Major code changes made to this file:
   The original file "sonic.java" was written in the java programming language.
   I have changed this file to work with the C# syntax with the intention to
...
public class Sonic {
    ...
    private const int SONIC_MIN_PITCH = 65;
    private const int SONIC_MAX_PITCH = 400;
    // This is used to down-sample some inputs to improve speed
    private const int SONIC_AMDF_FREQ = 4000;
    // The number of points to use in the sinc FIR filter for resampling.
    private const int SINC_FILTER_POINTS = 12;
    private const int SINC_TABLE_SIZE = 601;
    ...
    private static readonly short[] sincTable = {
        ...
        int newLength)
    {
        ...
        Array.Copy(oldArray, 0, newArray, 0, length);
        return newArray;
    }
    ...
    private void move(
        ...
        int numSamples)
    {
        Array.Copy(source, sourcePos*numChannels, dest, destPos*numChannels, numSamples*numChannels);
    }
    ...
    private bool prevPeriodBetter(
        int minDiff,
        int maxDiff,
        bool preferNewPeriod)
    {
        ...
        int rampUpPos)
    {
         for(int i = 0; i < numChannels; i++) {
            ...
            for(int t = 0; t < numSamples; t++) {
                output[o] = (short)((rampDown[d]*(numSamples - t) + rampUp[u]*t)/numSamples);
                o += numChannels;
                d += numChannels;
                u += numChannels;
            }
        ...
        int rampUpPos)
    {
        for(int i = 0; i < numChannels; i++) {
            ...
            for(int t = 0; t < numSamples + separation; t++) {
                if(t < separation) {
                    output[o] = (short)(rampDown[d]*(numSamples - t)/numSamples);
                    d += numChannels;
                } else if(t < numSamples) {
                    output[o] = (short)((rampDown[d]*(numSamples - t) + rampUp[u]*(t - separation))/numSamples);
                    d += numChannels;
                    u += numChannels;
                } else {
                    output[o] = (short)(rampUp[u]*(t - separation)/numSamples);
                    u += numChannels;
                }
                o += numChannels;
            }
        ...
        int newSampleRate)
    {
        ...
        for (i = 0; i < SINC_FILTER_POINTS; i++) {
            ...
            /* printf("%u %f\n", i, weight); */
            value = input[inPos + i*numChannels]*weight;
        ...
        for(position = 0; position < numPitchSamples - N; position++) {
            ...
            if(oldRatePosition == oldSampleRate) {
                ...
                if(newRatePosition != newSampleRate) {
                    throw new Exception("Assertion failed: newRatePosition != newSampleRate\n");
                }
                newRatePosition = 0;
            }
        }
